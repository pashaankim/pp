    ...
    for i, (title, cat, fid) in enumerate(quick_links):
        with cols[i % 4]:
            with st.container():
                ...
                """, unsafe_allow_html=True)
                if fid and st.button(f"Buka →", key=f"quick_{fid}"):
                    st.session_state.selected_formula = fid
                    st.rerun()
                elif not fid and cat == "Alat" and st.button(f"Buka →", key="quick_conv"):
                    st.rerun()
...
# ==================== CHEAT SHEET ====================
    ...
    with tab2:
        ...
        for sym, info in CONSTANTS.items():
            const_data.append([sym, info['name'], f"{info['value']:.3e}" if info['value'] < 0.01 else str(info['value']), info['unit']])
        st.table(const_data)
